...
for identifier in data['Identifier'].unique():
    ...
    table_rows2 = [[str(i+1), location_data.iloc[i]['LithologyName']] for i in range(len(location_data))]
    table_headers2 = ['Column 1', 'Column 2']
    tip_rows = ['<tr>{}</tr>'.format(''.join(['<td style="border: 1px solid black; padding: 5px;">{}</td>'.format(cell) for cell in row])) for row in table_rows]
    tooltip_rows2 = ['<tr>{}</tr>'.format(''.join(['<td style="border: 1px solid black; padding: 5px;">{}</td>'.format(cell) for cell in row])) for row in table_rows2]
    tooltip = '<table style="font-size: 14px; border-collapse: collapse; display: inline-block;">{}<tbody>{}</tbody></table>'.format(
        '<tr>{}</tr>'.format(''.join(['<th style="border: 1px solid black; padding: 5px;">{}</th>'.format(header) for header in table_headers])),
        ''.join(tooltip_rows)) + \
        '<table style="font-size: 14px; border-collapse: collapse; display: inline-block;">{}<tbody>{}</tbody></table>'.format(
        '<tr>{}</tr>'.format(''.join(['<th style="border: 1px solid black; padding: 5px;">{}</th>'.format(header) for header in table_headers2])),
        ''.join(tooltip_rows2))
